...
class OllamaClient:
    ...
    def ask(self, user_input: str, history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        logger.debug(
            "AI request_start request_id=%s operation=chat model=%s timeout=%s",
            request_id,
            self.model,
            self.timeout,
        )
        ...
        try:
            ...
            logger.debug(
                "AI response_received request_id=%s elapsed_ms=%.0f",
                request_id,
        ...
        try:
            data = response.json()
            logger.debug(
                "AI json_parsed request_id=%s elapsed_ms=%.0f",
                request_id,
                (time.perf_counter() - request_started) * 1000,
            )
