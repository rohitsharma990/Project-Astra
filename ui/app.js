...
// DIAGNOSTICS: Scene inspection
let gltfLoadCount = 0;
let renderLoopCount = 0;
let pointerEventCount = { down: 0, move: 0, up: 0, cancel: 0 };

function inspectScene(phase) {
  if (!scene) return;
  console.log(`[SCENE_INSPECT] ${phase} | children=${scene.children.length}`);
  scene.children.forEach((child, idx) => {
    console.log(`  [${idx}] ${child.constructor.name} name="${child.name}" visible=${child.visible} position=(${child.position.x.toFixed(2)},${child.position.y.toFixed(2)},${child.position.z.toFixed(2)})`);
  });
}

function countModelInstances() {
  if (!scene) return 0;
  let count = 0;
  scene.traverse(node => {
    if (node.isMesh && (node.name.toLowerCase().includes('ai_ohto') || node.name.toLowerCase().includes('armature'))) {
      count++;
    }
  });
  return count;
...
window.astraDiagnostics = function() {
  console.log('\n=== COMPREHENSIVE DIAGNOSTICS ===');
  console.log(`[GLTF_LOAD_COUNT] ${gltfLoadCount}`);
  console.log(`[RENDER_LOOP_COUNT] ${renderLoopCount}`);
  console.log(`[POINTER_EVENT_DOWN] ${pointerEventCount.down}`);
  console.log(`[POINTER_EVENT_MOVE] ${pointerEventCount.move}`);
  console.log(`[POINTER_EVENT_UP] ${pointerEventCount.up}`);
  console.log(`[POINTER_EVENT_CANCEL] ${pointerEventCount.cancel}`);
  console.log(`[SCENE_CHILDREN_COUNT] ${scene?.children?.length || 0}`);
  console.log(`[MODEL_INSTANCE_COUNT] ${countModelInstances()}`);
  console.log(`[ANIMATION_CONTROLLER_EXISTS] ${animationController ? 'YES' : 'NO'}`);
  console.log(`[ANIMATION_STATE] ${animationController?.state || 'N/A'}`);
  console.log(`[ANIMATION_DRAGGING] ${animationController?.dragging || false}`);
  
  if (model) {
    console.log(`[MODEL_POSITION] (${model.position.x.toFixed(2)}, ${model.position.y.toFixed(2)}, ${model.position.z.toFixed(2)})`);
    console.log(`[MODEL_ROTATION] (${model.rotation.x.toFixed(2)}, ${model.rotation.y.toFixed(2)}, ${model.rotation.z.toFixed(2)})`);
    console.log(`[MODEL_SCALE] (${model.scale.x.toFixed(2)}, ${model.scale.y.toFixed(2)}, ${model.scale.z.toFixed(2)})`);
  }
  
  console.log('[DIAGNOSTIC_FLAGS]');
  console.log(`  DISABLE_THREE_JS_TRANSFORMS_DURING_DRAG: ${window.DISABLE_THREE_JS_TRANSFORMS_DURING_DRAG ? 'YES' : 'NO'}`);
  console.log(`  DISABLE_ANIMATION_MIXER_DURING_DRAG: ${window.DISABLE_ANIMATION_MIXER_DURING_DRAG ? 'YES' : 'NO'}`);
  
  console.log('[SCENE_HIERARCHY]');
  scene?.traverse((node, idx) => {
    const indent = '  '.repeat(node.userData?._depth || 0);
    if (node.isMesh || node.name !== '') {
      console.log(`${indent}${node.constructor.name} "${node.name}" visible=${node.visible}`);
    }
  });
...
class AnimationController {
  ...
  update(delta, lookX) {
    // DIAGNOSTIC: Control animation mixer
    if (this.mixer) {
      // If mixer is disabled during drag, skip it
      if (window.DISABLE_ANIMATION_MIXER_DURING_DRAG && this.dragging) {
        // Skip mixer update, log if first skip
        if (!this._mixerSkipLogged) {
          console.log('[DIAGNOSTIC] Animation mixer skipped during drag');
          this._mixerSkipLogged = true;
        }
      } else {
        // Normal mixer update
        this.mixer.update(delta);
        this._mixerSkipLogged = false;
      }
    }

    if (this.dragging || this.state === 'dropping') {
      // DIAGNOSTIC: Control Three.js model transforms
      if (!window.DISABLE_THREE_JS_TRANSFORMS_DURING_DRAG) {
        // Normal transform updates
        this.root.position.x = this.basePosition.x;
        this.root.position.y = this.basePosition.y;
        this.root.position.z = this.basePosition.z;
        this.root.rotation.x = THREE.MathUtils.damp(this.root.rotation.x, this.dragVelocity.y * 0.08, 10, delta);
        this.root.rotation.z = THREE.MathUtils.damp(this.root.rotation.z, -this.dragVelocity.x * 0.12, 10, delta);
      } else {
        // Diagnostic: FREEZE model completely during drag
        if (!this._transformFreezeLogged) {
          console.log('[DIAGNOSTIC] Three.js model transforms frozen during drag');
          this._transformFreezeLogged = true;
        }
        // Position and rotation remain at their base values
        // Model stays completely stationary in viewport
      }
    ...
    this._transformFreezeLogged = false;
...
new GLTFLoader().load('models/ai_ohto1_clean.glb', gltf => {
  gltfLoadCount++;
  console.log(`[GLTF_LOAD] #${gltfLoadCount} starting load`);
  model = gltf.scene;
...
function animate() {
  ...
  if (renderLoopCount === 1) {
    console.log(`[RENDER_LOOP] started`);
  }
...
canvas.addEventListener('pointermove', event => {
  pointerEventCount.move++;
  pointer.x = (event.clientX / innerWidth) * 2 - 1;
  pointer.y = -(event.clientY / innerHeight) * 2 + 1;
  if (event.pointerId !== activePointerId) return;
  if (dragging && animationController?.dragging) animationController.moveGrab(canvasPoint(event));
  if (dragging && host && typeof host.move_companion_to_cursor === 'function') {
    // Pass screen coordinates (stable, not viewport-relative) for synchronized dragging
    host.move_companion_to_cursor(Math.round(event.screenX), Math.round(event.screenY));
  }
});
...
canvas.addEventListener('pointerdown', event => {
  pointerEventCount.down++;
  if (event.button !== 0) return;
  activePointerId = event.pointerId;
  dragging = true;
  console.log(`[DRAG_EVENT] pointerdown #${pointerEventCount.down} screen=(${event.screenX},${event.screenY})`);
  inspectScene('DRAG_START');
  ...
  if (host && typeof host.begin_drag === 'function') {
    // Pass screen coordinates (stable, not viewport-relative) to establish anchor point
    host.begin_drag(Math.round(event.screenX), Math.round(event.screenY));
  }
  logUi('drag started', { pointer: { x: event.screenX, y: event.screenY } });
  canvas.classList.add('dragging');
  canvas.setPointerCapture(event.pointerId);
});
function endDrag(event) {
  if (event.pointerId !== activePointerId) return;
  if (event.type === 'pointerup') pointerEventCount.up++;
  if (event.type === 'pointercancel') pointerEventCount.cancel++;
  console.log(`[DRAG_EVENT] ${event.type} screen=(${event.screenX},${event.screenY})`);
  inspectScene('DRAG_END');
  dragging = false;
...
// DIAGNOSTICS: Global functions for manual inspection
window.astraDiagnostics = () => {
  console.log('=== ASTRA DIAGNOSTICS ===');
  console.log(`[LOAD_COUNT] GLTF loads: ${gltfLoadCount}`);
  console.log(`[RENDER_LOOP] render calls: ${renderLoopCount}`);
  console.log(`[POINTER_EVENTS] down=${pointerEventCount.down} move=${pointerEventCount.move} up=${pointerEventCount.up} cancel=${pointerEventCount.cancel}`);
  inspectScene('MANUAL_INSPECTION');
  console.log(`[ANIMATOR] ${animationController ? 'LOADED' : 'NOT_LOADED'} state=${animationController?.state}`);
  console.log(`[DRAGGING] dragging=${dragging} activePointerId=${activePointerId}`);
};
