...
from .ai.user_facts import get_user_fact, get_user_facts_summary, remove_user_fact
from . import ui
...
def _handle_memory(words):
    ...
    if _starts_with(words, "what", "do", "remember") or _starts_with(words, "what", "remember"):
        ui.assistant_message(get_user_facts_summary())
        return True

    if _starts_with(words, "what", "is", "name"):
        name = get_user_fact("name") or get_user_fact("preferred_name")
        ui.assistant_message(f"Your name is {name}." if name else "I don't know your name.")
        return True

    if _starts_with(words, "what", "project") or _starts_with(words, "what", "am", "working", "on"):
        project = get_user_fact("project")
        ui.assistant_message(
            f"You're working on {project}." if project else "I don't know what project you're working on."
        )
        return True

    if _starts_with(words, "forget", "name"):
        if remove_user_fact("name"):
            ui.assistant_message("I forgot your name.")
        else:
            ui.assistant_message("I don't know your name.")
        return True

    if (
        _starts_with(words, "clear", "memory")
        or _starts_with(words, "clear", "conversation", "memory")
